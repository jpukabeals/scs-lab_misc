...
#yield
ydat<-read.xlsx("IR4-All-2020-data.xlsx", sheet=9, startRow=11, na.strings="NA", cols=1:10)
str(ydat)
ydat<-ydat%>%
  mutate_if(is.character,as.factor)
...
mod3<-lm(yield~fherb, na.action=na.omit, 
         data=subset(ydat2, stand=="Old"&
                       state=="WI"))
cld(emmeans(mod3, ~fherb|spray_date), sort=F, Letters=letters)
...
idatnsum<-summarySE(subset(idat, stand=="New"&spray_date=="Spring 2020"), "iwg_injury", c("state", "fherb", "sample_season"), na.rm=T)
...
ggplot(idatnsum, aes(x=herbicide, y=iwg_injury, fill=rate))+
  facet_grid(state~spray_date)+
