...
dat_yield %>% 
  ...
  knitr::kable(
    caption = "Wheatgrass yields did not differ among different PRE applications"
  )
```
...
dat_shattering %>% 
  ...
  mutate(shattering = paste(round(shattering,0),
                        group,
                        sep = " ")) %>% 
  dplyr::rename(Shattering = shattering) %>% 
  dplyr::select(POST,Shattering,n) %>% 
  knitr::kable(
    caption = "Wheatgrass shattering scores where 0 is no shattering and 10 is a lot of shattering. Data collected in April 2023. Count data was also collected, but not presented. An alpha level of 0.1 was used"
  )
```
